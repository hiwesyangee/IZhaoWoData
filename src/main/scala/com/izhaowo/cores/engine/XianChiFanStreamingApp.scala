...
object XianChiFanStreamingApp {
  ...
  def runningStreaming(): Unit = {
    ...
    ssc.checkpoint("hdfs://master:8020/opt/checkpoint/stream1.3")

    val kafkaParams = Map[String, Object](
      "bootstrap.servers" -> JavaXianChiFanProperties.BOOTSTARP_SERVERS,
      "key.deserializer" -> classOf[StringDeserializer],
      "value.deserializer" -> classOf[StringDeserializer],
      "group.id" -> JavaXianChiFanProperties.GROUP,
      "auto.offset.reset" -> "latest",
      "enable.auto.commit" -> (false: java.lang.Boolean)
    )

    val topics = JavaXianChiFanProperties.StringTOPIC
