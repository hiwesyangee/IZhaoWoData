...
/**
  * 数据验证03辅助类。
  */
object DataValidation03Util {

  // 原始系数数组
  val coefficientArr = Array(0.05013d, 0.135d, 0.05027d, 0.11718d, 0.06183d, 0.02322d, 0.135d, 0.27711d, (0.1d / 156d), 0.05027d)
  ...
  def legalPlannerFilter(lp: LegalPlanner): LegalPlanner = {
    ...
    if (lp.case_dot >= -0.8d && lp.case_dot <= 1) { // 1.number=16
      if (lp.reorder_rate >= -1 && lp.reorder_rate <= 1) { // 2.number=16
        if (lp.communication_level >= -0.9d && lp.communication_level <= 0.2d) { // 3.number=13
          if (lp.design_sense >= -0.75d && lp.design_sense <= 0.6d) { // 4.number=2
            if (lp.case_rate >= -1 && lp.case_rate <= 1) { // 5.number=1
              if (lp.all_score_final >= 0 && lp.all_score_final <= 1) { // 6.number=1
                if (lp.number >= -1d && lp.number <= 0.2d) { // 7.number=1
                  if (lp.text_rating_rate >= -0.9d && lp.text_rating_rate <= 1) { // 8.number=1
                    if (lp.display_amount >= 900 && lp.display_amount <= 1500) { // 9.number=0
                      if (lp.to_store_rate >= -0.5d && lp.to_store_rate <= 1) { // 10.number=0[1]
                        bool_res = true
                      }
                    }
                  }
  ...
  def getTotalDataFromDiffFormula(number: Int, calculateData: Double, originalData: Double): Double = {
    ...
    number match {
      case 1 => { // 针对公式1进行计算
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("1", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(0) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 2 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("2", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(1) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 3 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("3", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(2) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 4 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("4", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(3) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 5 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("5", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(4) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 6 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("6", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(5) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 7 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("7", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(6) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 8 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("8", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(7) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 9 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("9", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
        end = coefficientArr(8) * DataValidationUtils.getIntegralResult(integralArr)
      }
      case 10 => {
        val integralArr = DataValidationUtils.getIntegralDataByInterface5001("10", s"[$calculateData]", "1") // 数值作为下限，进行计算积分
