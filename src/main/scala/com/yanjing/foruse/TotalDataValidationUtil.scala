...
object TotalDataValidationUtil {
  ...
  def legalPlannerFilter(lp: LegalPlanner): LegalPlanner = {
    //        var bool_res = true
    var bool_res = false
    if (lp.case_dot >= -0.57d && lp.case_dot <= 1) { // 1.number=16
      if (lp.reorder_rate >= -1 && lp.reorder_rate <= 1) { // 2.number=16
        if (lp.communication_level >= -1d && lp.communication_level <= 1) { // 3.number=13
          if (lp.design_sense >= 0.3d && lp.design_sense <= 1) { // 4.number=2
            if (lp.case_rate >= -0.99d && lp.case_rate <= 1) { // 5.number=1
              if (lp.all_score_final >= 0.9d && lp.all_score_final <= 1) { // 6.number=1
                if (lp.number >= -0.9d && lp.number <= 0) { // 7.number=1
                  if (lp.text_rating_rate >= -0.45d && lp.text_rating_rate <= 1) { // 8.number=1
                    if (lp.display_amount >= 1200 && lp.display_amount <= 2000) { // 9.number=0
