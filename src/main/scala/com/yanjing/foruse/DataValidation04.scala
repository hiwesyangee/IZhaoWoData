...
import com.yanjing.foruse.DataValidation04Util.LegalPlanner
import org.apache.log4j.{Level, Logger}

/**
  * 数据验证，策略源于2019.09.25肖老师给定策略。【成都瑞城名人酒店-3】
  */
object DataValidation04 {
  ...
  def doDataValidation(hotel_id: String, hotel_name: String, section: Int): Unit = {
    ...
    for (row <- arr) {
      ...
      // 返回新对象。
      val newLP = DataValidation04Util.legalPlannerFilter(lp)
      if (newLP != null) {
        set = set.+(newLP)
      }
      allSet = allSet.+(lp)
    ...
    val lastPlannerSet: Set[LegalPlanner] = DataValidation04Util.noSchedulePlannerFilter(set, "20190923")
    ...
    println("===========================")
    val fv16 = DataValidation04Util.getTenDataByStatistical2(lastPlannerSet)
    val fv17 = DataValidation04Util.getTenDataByStatistical(lastPlannerSet)
    println()
    println(fv16)
    println(fv17)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    doDataValidation("f8655d30-1153-4622-9a43-0e43e544d7e9", "成都瑞城名人酒店", 3)
