...
import com.yanjing.foruse.DataValidation03Util.LegalPlanner
import org.apache.log4j.{Level, Logger}

/**
  * 数据验证，策略源于2019.09.25肖老师给定策略。【索菲斯锦苑宾馆(武都路4号院南)-3】
  */
object DataValidation03 {
  ...
  def doDataValidation(hotel_id: String, hotel_name: String, section: Int): Unit = {
    ...
    for (row <- arr) {
      ...
      // 返回新对象。
      val newLP = DataValidation03Util.legalPlannerFilter(lp)
      if (newLP != null) {
        set = set.+(newLP)
      }
      allSet = allSet.+(lp)
    ...
    val lastPlannerSet: Set[LegalPlanner] = DataValidation03Util.noSchedulePlannerFilter(set, "20191021")
    ...
    println("===========================")
    //    val data4Original = DataValidation03Util.getTenDataByStatistical(lastPlannerSet)
    //    val data4Contraction = DataValidation03Util.getTenDataByStatistical2(lastPlannerSet)
    //    val data4Integral = DataValidation03Util.getTenDataByStatistical3(lastPlannerSet)

    //    println(data4Original)
    //    println(data4Contraction)
    //    println(data4Integral)

    val data4Predict = DataValidationUtils.getPredictNumberFromPythonInterface("30", "", "")
  ...
  def main(args: Array[String]): Unit = {
    ...
    doDataValidation("19b725d8-866f-4433-ac93-d85eabc866b3", "索菲斯锦苑宾馆(武都路4号院南)", 3)
