package com.statusCheck.client

import java.text.SimpleDateFormat
import java.util.{Date, Timer, TimerTask}

import com.izhaowo.cores.utils.JavaDateUtils
import com.statusCheck.engine.StatusCheckEngine
import org.apache.log4j.{Level, Logger}

object StatusCheckClient {
  ...
  def timerRun(): Unit = {
    ...
    // 规定的每天时间00:30:00运行
    val sdf = new SimpleDateFormat("yyyy-MM-dd 02:00:00") // 每天两点定时执行。
    ...
    try {
      ...
      val task = new TimerTask() {
        override def run(): Unit = { // 0.打印当前时间
          ...
          StatusCheckEngine.SmallMarketStatus
          val stop2 = System.currentTimeMillis

          System.out.println(today + " 每日定时任务——小市场状态Check,用时: " + (stop2 - stop) / 1000 + "s.")
        }
      }
      // 以每24小时执行一次
